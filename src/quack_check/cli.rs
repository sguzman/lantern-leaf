...
fn classify(cfg: &Config, input: &Path) -> Result<()> {
    let engine = PythonEngine::new(cfg)?;
    let probe = crate::quack_check::probe::probe_pdf(cfg, &engine, input)?;
    let decision = crate::quack_check::policy::decide(cfg, &probe);
    println!(
        "{}",
...
fn plan(cfg: &Config, input: &Path) -> Result<()> {
    let engine = PythonEngine::new(cfg)?;
    let probe = crate::quack_check::probe::probe_pdf(cfg, &engine, input)?;
    let plan = crate::quack_check::chunk_plan::ChunkPlan::from_probe(cfg, &probe)?;
...
fn run(args: &Args, cfg: &Config, input: &Path, out_override: Option<&Path>) -> Result<()> {
    ...
    let input_hash = crate::quack_check::util::hash_file(cfg, input)
