...
- [ ] P14-02 Complete parity signoff checklist with explicit pass on all must-have behaviors. (In progress: expanded runtime smoke now covers search + full TTS control surface + clipboard/recent flows + EPUB open runtime coverage + PDF terminal event lifecycle checks; remaining gaps are calibre-heavy runtime scenarios, full PDF-success soak across environments, and final soak.)
...
- [ ] R-06 Tauri permission/capability restrictions breaking filesystem/subprocess workflows. (Mitigated in part by root-aware config path resolution for pandoc/quack-check when running under `src-tauri` manifest context.)
