...
pub const PDF_ZOOM_LEVELS: [f32; 15] = [
    0.25, 0.35, 0.5, 0.67, 0.8, 0.9, 1.0, 1.1, 1.25, 1.5, 1.75, 2.0, 2.5, 3.0, 4.0,
];
...
impl Default for PdfZoomState {
    fn default() -> Self {
        Self {
            mode: PdfZoomMode::Manual,
            manual_level: PDF_DEFAULT_ZOOM_LEVEL,
        }
    }
}
...
impl PdfZoomState {
    pub fn manual(level: f32) -> Self {
        Self {
            mode: PdfZoomMode::Manual,
            manual_level: level.clamp(0.25, 4.0),
        }
    }
    ...
    pub fn effective_level(
        ...
        match self.mode {
            ...
            PdfZoomMode::FitWidth => {
                (available_width.max(1.0) / page_width.max(1.0)).clamp(0.25, 4.0)
            }
            PdfZoomMode::FitPage => (available_width.max(1.0) / page_width.max(1.0))
                .min(available_height.max(1.0) / page_height.max(1.0))
                .clamp(0.25, 4.0),
        }
    }
}
...
mod tests {
    ...
    fn practical_zoom_supports_fit_modes_and_wider_manual_range() {
        ...
        let width = PdfZoomState {
            mode: PdfZoomMode::FitWidth,
            manual_level: 1.0,
        };
        assert_eq!(width.effective_level(800.0, 600.0, 400.0, 800.0), 2.0);
        let page = PdfZoomState {
